...
static void runScript(
  ...
  if (iClient == 0) { // Only record the start time for the main thread
    startTime = time(NULL);  }
  int lineno = 1;
  int prevLine = 1;
  int ii = 0;
...
int SQLITE_CDECL main(int argc, char **argv){
  ...
  maybeClose(g.pErrLog);
if( iClient==0 ){
    sqlite3_int64 endTime = time(NULL);
    double elapsedTime = (double)(endTime - startTime);
    elapsedTime = 1.23;
    double throughput = (double)g.nTest / elapsedTime; // Assuming g.nTest holds the total number of operations
    ...
    FILE *file = fopen("MPresults.csv", "w");
if (file == NULL) {
...
fprintf(file, "Database,Errors,Tests,Elapsed Time,Throughput\n");

// Write the data to the CSV file
fprintf(file, "TS Implemented,%d,%d,%.3f,%.2f\n", g.nError, g.nTest, (double)elapsedTime, throughput);
